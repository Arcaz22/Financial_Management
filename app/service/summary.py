    ...
    async def get_monthly_summary(self, year: int, month: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not worksheet:
                ...
                transaction_sheets = [sheet for sheet in all_sheets if sheet.title.startswith("Transaksi")]
                ...
                transaction_sheets.sort(key=lambda s: int(s.title.split()[-1]) if s.title.split()[-1].isdigit() else 0, reverse=True)
                worksheet = transaction_sheets[0]
                year = int(worksheet.title.split()[-1]) if worksheet.title.split()[-1].isdigit() else year
                logger.info(f"Using alternative sheet: {worksheet.title} for requested year {year}")

            # Get all data
            all_data = worksheet.get_all_values()
            logger.info(f"Retrieved {len(all_data) - 1 if len(all_data) > 0 else 0} data rows")
            ...
            logger.info(f"Column indices - Date: {date_idx}, Amount: {amount_idx}, Type: {type_idx}, Category: {category_idx}")
            ...
            for row_idx, row in enumerate(data):
                if len(row) <= max(date_idx, amount_idx, type_idx, category_idx):
                    logger.warning(f"Row {row_idx + 2} skipped: not enough columns. Row data: {row}")
                ...
                try:
                    ...
                    if date_obj:
                        logger.info(f"Date '{date_str}' parsed with Indonesian format => {date_obj}")
                    else:
                        formats_tried = []
                        ...
                        for date_format in formats_to_try:
                            try:
                                ...
                                logger.info(f"Date '{date_str}' parsed with format '{date_format}' => {date_obj}")
                                break
                            except ValueError:
                                continue

                    if not date_obj:
                        logger.warning(f"Couldn't parse date: '{date_str}'. Tried formats: {formats_tried}")
                        continue

                    row_month = date_obj.month
                    row_year = date_obj.year
                    logger.info(f"Date info - month: {row_month}, year: {row_year} (looking for month: {month}, year: {year})")
            ...
            logger.info(f"Filtered data: found {len(month_data)} transactions for {month}/{year}")
            ...
            for row_idx, row in enumerate(month_data):
                try:
                    ...
                    logger.info(f"Processing amount for row {row_idx + 1}: '{amount_str}' => {amount}")
                    ...
                    if transaction_type == "pemasukan":
                        total_income += amount
                        logger.info(f"Added to income: {amount} (total: {total_income})")
                    else:  # Pengeluaran
                        total_expense += amount
                        logger.info(f"Added to expense: {amount} (total: {total_expense})")
                        ...
                        logger.info(f"Updated category '{category}': {category_totals[category]}")

                except (ValueError, IndexError) as e:
                    logger.error(f"Error processing amount or category in row {row_idx + 1}: {e}")
                    continue
            ...
            analysis.append(f"💰 *Total Pemasukan:* Rp {total_income:,}".replace(",", "."))
            analysis.append(f"💸 *Total Pengeluaran:* Rp {total_expense:,}".replace(",", "."))

            # Calculate balance
            balance = total_income - total_expense
            balance_status = "🟢 SURPLUS" if balance >= 0 else "🔴 DEFISIT"
            analysis.append(f"🧮 *Saldo:* Rp {balance:,} ({balance_status})".replace(",", "."))
            ...
            if sorted_categories:
                ...
                for category, amount in sorted_categories[:5]:  # Show top 5 categories
                    ...
                    analysis.append(f"• {category}: Rp {amount:,} ({percentage:.1f}%)".replace(",", "."))

                if len(sorted_categories) > 5:
                    other_total = sum(amount for _, amount in sorted_categories[5:])
                    other_percentage = (other_total / total_expense * 100) if total_expense > 0 else 0
                    analysis.append(f"• Lainnya: Rp {other_total:,} ({other_percentage:.1f}%)".replace(",", "."))
            ...
            logger.info(f"Summary generated successfully - Income: {total_income}, Expense: {total_expense}")
