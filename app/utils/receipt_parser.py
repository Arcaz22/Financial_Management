...
class ReceiptParser:
    ...
    def parse_receipt_text(self, text: str) -> dict:
        ...
        if transaction_data["tanggal"]:
            try:
                ...
                transaction_data["tanggal"] = f"{date_obj.day} {month_names[date_obj.month-1]} {date_obj.year} {datetime.now().hour:02d}:{datetime.now().minute:02d}"
            except:
                now = datetime.now()
                month_names = [
                    "Januari", "Februari", "Maret", "April", "Mei", "Juni",
                ...
                transaction_data["tanggal"] = f"{now.day} {month_names[now.month-1]} {now.year} {now.hour:02d}:{now.minute:02d}"
        else:
            now = datetime.now()
            month_names = [
                "Januari", "Februari", "Maret", "April", "Mei", "Juni",
    ...
    def _extract_store_name(self, text: str) -> str:
        ...
        for i in range(min(3, len(lines))):
            if lines[i] and not any(keyword in lines[i].lower() for keyword in ['receipt', 'invoice', 'struk', 'nota']):
    ...
    def _extract_category(self, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ['restaurant', 'resto', 'cafe', 'food', 'makanan', 'minuman']):
            return "Makanan"
        elif any(keyword in text_lower for keyword in ['transport', 'transportasi', 'grab', 'gojek', 'taxi', 'taksi', 'bus', 'train', 'kereta']):
            return "Transportasi"
        elif any(keyword in text_lower for keyword in ['invest', 'investasi', 'saham', 'reksadana', 'obligasi']):
            return "Investasi"
        elif any(keyword in text_lower for keyword in ['belanja', 'supermarket', 'mart', 'market', 'toko', 'retail']):
            return "Belanja"
        elif any(keyword in text_lower for keyword in ['pulsa', 'data', 'internet', 'telepon', 'phone']):
    ...
    def _extract_items(self, text: str) -> list:
        ...
        for i in range(len(lines)):
            price_match = re.search(r'(.*?)(?:Rp\.?|IDR)?\s*([\d.,]+)(?:\s*(?:x|×)\s*(\d+))?\s*$', lines[i])
            if price_match:
                ...
                if (name and not any(keyword in name.lower() for keyword in
                                    ['total', 'subtotal', 'tax', 'pajak', 'diskon', 'discount'])):
                    items.append({
                        'name': name,
                        'price': price,
                        'quantity': quantity
